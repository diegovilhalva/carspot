...
const FormGenerator: React.FC<FormGeneratorProps> = ({ field, register, formValue, onChange, defaultValue, valueMultiSelect }) => {
    const { name, fieldType, label, disabled, placeholder, options } = field

    const getSelectedItems = (options: { key?: string; label: string; value: string; }[], valueMultiSelect: string[] = []) => {
                ...
                fieldType === "currency" && (
                    <div className="relative">
                        ...
                        <Input
                            ...
                            {...register(name, {
                                onChange: (e) => {
                                    const rawValue = e.target.value ? e.target.value.replace(/\D/g, "") : ""
                                    const formattedValue = rawValue ? new Intl.NumberFormat().format(Number(rawValue)) : ""
