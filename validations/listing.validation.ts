import { CAR_BODY_TYPE_OPTIONS, CAR_BRAND_OPTIONS, CAR_COLOR_OPTIONS, CAR_CONDITION_OPTIONS, CAR_DRIVETRAIN_OPTIONS, CAR_FUELTYPE_OPTIONS, CAR_KEY_FEATURES_OPTIONS, CAR_MODEL_OPTIONS, CAR_SECOND_CONDITION_OPTIONS, CAR_TRANSMISSION_OPTIONS, CAR_YEAR_OPTIONS } from "@/constants/car-options";
import { z } from "zod";
const createEnum = (options: { value: string }[], fieldName: string) => {
    const values = options.map((item) => item.value)
    if (values.length === 0) {
        throw new Error(`No option found for ${fieldName}`)
    }

    return z.enum([values[0], ...values.splice(1)], {
        errorMap: (issue, ctx) => {
