...
import { createAnounymousClient } from "@/lib/appwrite";
...
export async function GET(req:NextRequest){
    try {
        ...
        const {databases} = await createAnounymousClient()

        const listings  = await databases.listDocuments(APP_CONFIG.APPWRITE.DATABASE_ID,APP_CONFIG.APPWRITE.CART_LISTING_ID,queries)
