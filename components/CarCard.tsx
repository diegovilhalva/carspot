...
const CarCard: FC<CarCardProps> = ({ listing, layout = "grid" }) => {
  ...
  const conditionLabel = CAR_CONDITION_OPTIONS.find((opt) => opt.value === condition)?.label
  return (
    <div>
      <Link href={`/detail/${slug}/${$id}`}>
        <Card className={cn(`border rounded-lg shadow-sm p-0 flex flex-col gap-4`, layout === "list" && "flex-row  border-primary/30 gap-0 md:gap-4")}>
          <div className={cn(`relative  w-full min-h-28 !h-[210px] bg-primary/10 overflow-hidden`, layout === "list" && `w-[152px]  !h-auto md:w-[220px] md:h-[210px] shrink-0`)}>
            <Image src={imageUrls[0]} alt="Car image" className={cn(`rounded-t-lg w-full h-full object-cover`, layout === "list" && "!rounded-r-none")}
              width={layout === "list" ? 300 : 800}
              height={layout === "list" ? 200 : 500} />
          </div>
          <CardContent
            className={cn(`!p-4 !pt-0 space-y-1`,
              layout === "list" && `flex-1 !p-[10px_16px_18px_16px] md:!p-[18px_16px_18px_0px]`
            )}>
            <div className="flex flex-col gap-0">
              <h3 className={cn(`font-bold text-xs sm:text-base  text-gray-800 capitalize truncate whitespace-normal`, layout === "list" && "w-auto tracking-tighter sm:tracking-normal")}>
                {displayTitle}
              </h3>
              <div className={cn(`h-auto mt-1 text-xs sm:text-sm text-gray-500
                         line-clamp-2 text-ellipsis`, layout === "list" && "w-auto h-auto whitespace-break-spaces")}>
                {description}
              </div>
            </div>
